...
export default function DemandForm(props) {
  ...
  const handleSubmit = async (event) => {
    try {
      ...
      if (mode === "add") await addDocument(demand);
      if (mode === "edit") await updateDocument(demand, selectedDemandId.current);
  ...
  const handleCurrencyInputEvent = (event) => {
    ...
    if (!currentValue) {
      ...
      setSelectedDemand({ ...selectedDemand, amount: 0 });
      return;
    }
  ...
  useEffect(() => {
    if (mode === "add") {
      if (localCountryInfo && !selectedDemand.currency) {
        setSelectedDemand({
          ...selectedDemand,
  ...
  const initDemandById = async () => {
    ...
    try {
      ...
      selectedDemandId.current = doc.id;

      // Init Form Refs values.
      amountRef.current.value = convertNumberWithCommas(Number(demand.amount));
      titleRef.current.value = demand.title;
    ...
    <div className="task-form">
      <div className="container">
        <div className="task-form__header">
          ...
          <h5 className="task-form__title">{`${mode} Demand`}</h5>
        </div>
          ...
          <Form onSubmit={handleSubmit}>
            <div className="mb-3">
              ...
              <div className="task-form__amount-input">
                ...
                >
                  {selectedDemand.currency &&
                    getSymbolByCurrency(selectedDemand.currency)}
                </span>
                <input
        ...
        {CurrencyModalShow && (
          <CurrencyModal
            amountRef={amountRef}
            setSelectedTask={setSelectedDemand}
            selectedTask={selectedDemand}
          />
        )}
      </div>
    </div>
